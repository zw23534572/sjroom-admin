...
public class UserServiceImpl extends BaseServiceImpl<IUserDao, User> implements IUserService {
	...
	public Map<Long, String> fillFieldName(Set<Long> bIds) {
		List<UserBo> userBos = this.findByBIds(bIds);
		if (CollectionUtil.isEmpty(userBos)) {
			log.warn("UserServiceImpl find userBos is empty");
			return Collections.emptyMap();
		}
		return userBos.stream().collect(Collectors.toMap(UserBo::getUserId, UserBo::getUserName));
	}
	...
	private LambdaQueryWrapper<User> query(UserBo model) {
	    LambdaQueryWrapper<User> wrapper = new LambdaQueryWrapper<User>();
			...
			wrapper.eq(ObjectUtil.isNotNull(model.getType()), User::getType, model.getType());
