...
public class UserRoleServiceImpl extends BaseServiceImpl<IUserRoleDao, UserRole> implements IUserRoleService {
	...
	public Map<Long, String> fillFieldName(Set<Long> bIds) {
		List<UserRoleBo> userRoleBos = this.findByBIds(bIds);
		if (CollectionUtil.isEmpty(userRoleBos)) {
			log.warn("UserRoleServiceImpl find userRoleBos is empty");
			return Collections.emptyMap();
		}
		return userRoleBos.stream().collect(Collectors.toMap(UserRoleBo::getUserRoleId, UserRoleBo::getUserRoleName));
	}
